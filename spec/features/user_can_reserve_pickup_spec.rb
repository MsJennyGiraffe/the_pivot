...
RSpec.feature "user can checkout" do
  ...
  scenario "user does not see reservation times taken" do
    ...
    formatted_existing_order_time = (existing_order_time1).strftime("%b %e, %I:%M %p")

    expect(order).to be_valid

    reservation1 = Reservation.create(pickup_time: existing_order_time1, order: order)
    reservation2 = Reservation.create(pickup_time: existing_order_time2, order: order)
    ...
    within "#reservation_pickup_time" do
      expect(page).to_not have_content(formatted_existing_order_time)
    end
