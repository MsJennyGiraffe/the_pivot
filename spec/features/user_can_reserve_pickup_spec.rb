...
RSpec.feature "user can checkout" do
  ...
  scenario "user does not see reservation times taken" do
    ...
    existing_order_time = DateTime.now.beginning_of_day + 12.hour + 1.day
    formatted_existing_order_time = (existing_order_time).strftime("%b %e, %I:%M %p")

    expect(order).to be_valid

    reservation = Reservation.create(pickup_time: existing_order_time, order: order)
    reservation.save
