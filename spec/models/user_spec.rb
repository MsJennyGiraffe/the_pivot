require 'rails_helper'

RSpec.describe User, type: :model do
  let(:user_instance) { build(:user) }

  it "has a valid factory" do
    expect(user_instance).to be_valid
    expect(user_instance).to respond_to(:username)
    expect(user_instance).to respond_to(:password_digest)
    expect(user_instance).to have_secure_password
    expect(user_instance).to respond_to(:email)
    expect(user_instance).to respond_to(:role)
  end

  describe "user validates attributes" do
    it { expect(user_instance).to validate_presence_of(:password_digest) }
    it { expect(user_instance).to validate_presence_of(:email) }
  end
end

  RSpec.describe "user can create name" do
    it "can create name and update role" do
      user = User.create(username: "hello", password: "password", email: "email@email.com", approved: false, requested: true)

      expect(user.role).to eq("default")

      user.update_role

      expect(user.role).to eq("seller")
      expect(user.approved).to eq(true)
      expect(user.requested).to eq(false)
      expect(user.name).to eq("email")
    end

    it "can update role if platform_admin" do
      user = User.create(username: "hello", password: "password", email: "email@email.com", role: 2, approved: false, requested: true)

      user.update_role

      expect(user.approved).to eq(true)
      expect(user.requested).to eq(false)
    end
  end
